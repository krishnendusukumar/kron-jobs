-- Clerk Integration Migration (Safe Version) - Handles Existing Data
...
-- 2. Create unique index for Clerk user ID (only if it doesn't exist)
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_user_profiles_clerk_user_id') THEN
...
-- 6. Create indexes for new tables (only if they don't exist)
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_user_usage_user_id') THEN
        CREATE INDEX idx_user_usage_user_id ON user_usage(user_id);
    END IF;
    IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_user_usage_created_at') THEN
        CREATE INDEX idx_user_usage_created_at ON user_usage(created_at);
    END IF;
    IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_billing_subscriptions_user_id') THEN
        CREATE INDEX idx_billing_subscriptions_user_id ON billing_subscriptions(user_id);
    END IF;
    IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_billing_subscriptions_stripe_customer_id') THEN
...
-- 8. Create RLS policies for user_usage (only if they don't exist)
DO $$
BEGIN
...
-- 9. Create RLS policies for billing_subscriptions (only if they don't exist)
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE policyname = 'Users can view own billing') THEN
        CREATE POLICY "Users can view own billing" ON billing_subscriptions
            FOR SELECT USING (user_id = current_user);
    END IF;
    IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE policyname = 'Users can update own billing') THEN
        CREATE POLICY "Users can update own billing" ON billing_subscriptions
            FOR UPDATE USING (user_id = current_user);
    END IF;
END $$;

-- 10. Create RLS policies for audit_logs (only if they don't exist)
DO $$
BEGIN
...
-- 11. Create function to track usage (replace if exists)
...
-- 12. Create function to get user usage summary (replace if exists)
...
-- 13. Create function to check user limits (replace if exists)
...
-- 14. Create triggers for audit logging (replace if exists)
...
-- 15. Apply audit triggers to key tables (drop and recreate to avoid conflicts)
...
DROP TRIGGER IF EXISTS audit_cron_jobs_changes ON cron_jobs;
...
-- 16. Create function to sync Clerk user data (replace if exists)
...
-- 17. Create function to reset credits daily (enhanced) - replace if exists
...
-- 18. Insert sample data for testing (only if not exists and with unique emails)
...
-- 19. Verify the migration completed successfully
